...
| **When does fusion stop paying?** | Experiment 20 found the first fusion here that does not beat its own components: BM25+ColBERT is −0.0046 against ColBERT alone on NFCorpus, where BM25+dense gained 0.0357 on the same data. The untested hypothesis is that RRF pays when components are comparably strong and costs when one is clearly weaker. Settling it needs a deliberate sweep of component strength, not another pair of models |
| **What is actually wrong with the MS MARCO models?** | Four models trained on it underperformed here and the explanation offered each time was domain mismatch. SPLADE is MS MARCO-trained and is the best single retriever measured here, so that explanation is dead and nothing has replaced it |
